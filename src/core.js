...
export const CATEGORIES = ['Transport', 'Home Energy', 'Food', 'Shopping', 'Travel'];
...
export function createInitialState() {
  ...
  return {
    ...
    goals: [
      { id: 'goal-transport', title: 'Reduce transport emissions by 20%', category: 'Transport', targetValue: 22, currentValue: 0, unit: 'kg CO2', deadline: todayKey, rewardXp: 120 },
      { id: 'goal-food', title: 'Have 5 plant-based meals this week', category: 'Food', targetValue: 5, currentValue: 0, unit: 'meals', deadline: todayKey, rewardXp: 90 }
    ],
    habits: seedHabits(),
    completedWeeklyPlan: [],
    reflections: [],
...
export function computeLevelFromXP(xp) {
  ...
  const progress = Math.round(clamp(((xp - current.min) / (nextThreshold - current.min)) * 100, 0, 100));
  return {
    level: current.level,
    title: current.title,
    progress,
...
function sum(items, key) {
  return round(items.reduce((total, item) => total + Number(item[key] || 0), 0), 1);
}
...
export function getTopCategory(entries) {
  const totals = CATEGORIES.map((category) => ({
    category,
    total: sum(entries.filter((entry) => entry.category === category), 'co2kg')
...
export function computeCarbonScore({ monthCO2, avoidedMonth = 0, streak = 0 }) {
  ...
  return Math.round(clamp(98 - monthCO2 * 0.26 + avoidedMonth * 0.2 + Math.min(10, streak * 0.45), 18, 100));
}
...
export function computeWeeklyData(entries, today = startOfToday()) {
  return Array.from({ length: 7 }, (_, index) => {
    ...
    const co2kg = sum(entries.filter((entry) => entry.date === key), 'co2kg');
    return {
      date: key,
...
export function computeCategoryBreakdown(entries) {
  ...
  return CATEGORIES.map((category) => {
    const kg = sum(entries.filter((entry) => entry.category === category), 'co2kg');
...
export function generateWeeklyPlan(state, topCategory = 'Transport') {
  ...
  const categories = [topCategory, 'Food', 'Transport', 'Home Energy', 'Shopping', 'Travel']
...
export function computeStats(state) {
  ...
  const streak = Math.min(30, new Set(state.entries.map((entry) => entry.date)).size);
...
export function computeCarbonBudget(state, stats = computeStats(state)) {
  ...
  return {
    ...
    projected: round((stats.monthCO2 / Math.max(1, today.getDate())) * daysInMonth, 1),
...
export function computeScenario(state, stats = computeStats(state)) {
  ...
  const lowCarbonWeek = round(Math.max(stats.weeklyTotal * 0.38, stats.weeklyTotal - planSavings), 1);
  const weeklySavings = round(Math.max(0, stats.weeklyTotal - lowCarbonWeek), 1);
  return {
...
export function updateBudget(state, value) {
  return {
    ...state,
    carbonBudget: {
      monthlyLimit: clamp(Number(value) || 160, 40, 800),
...
export function completePlanAction(state, id) {
  ...
  }, item.rewardXp, item.title.toLowerCase());
}
...
export function saveReflection(state, mood, note) {
  ...
  return awardXP({
    ...state,
    reflections: [...state.reflections.filter((item) => item.date !== todayKey), reflection],
    selectedReflectionMood: reflection.mood
  }, 22, 'saving a daily reflection');
}
...
export function calculateEntryCO2(category, subtype, quantity) {
  ...
  const unit = category === 'Food' ? 'meal' : category === 'Home Energy' ? (subtype === 'Natural gas' ? 'm3' : 'kWh') : category === 'Transport' ? 'km' : 'action';
  return { co2kg: round(amount * factor, 1), unit };
}

export function addActivityEntry(state, draft) {
  const category = CATEGORIES.includes(draft.category) ? draft.category : 'Transport';
  const subtype = sanitizeText(draft.subtype, 80) || 'Train';
  const quantity = Math.max(0, Number(draft.quantity) || 1);
  ...
  return awardXP({ ...state, entries: [...state.entries, entry] }, impact.co2kg <= 2 ? 42 : 30, 'logging activity');
}
  ...
  switch (action.type) {
    ...
    case 'applyScenario': {
      ...
      return awardXP({
        ...
        currentView: 'goals'
      }, 28, 'applying a low-carbon scenario');
    }
    case 'toast':
